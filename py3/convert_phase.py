...
class UserAvClip:
        ...
        with open(self.uid_file) as f:
            ...
            for line in lines:
                ...
                if items[0] == "audio":
                    ...
                    if items[3] == "create":
                        self.audio_clip.start_time[index] = float(items[1])
                    elif items[3] == "close":
                        self.audio_clip.end_time[index] = float(items[1])

                if items[0] == "video":
                    index = self.clip.put_file(items[2])
                    if items[3] == "create":
                        self.clip.start_time[index] = float(items[1])
                    elif items[3] == "close":
                        self.clip.end_time[index] = float(items[1])
    ...
    def convert(self, suffix, offset_time):
        ...
        if self.clip.num > 0:
            ...
            print("Output resolution: %dx%d" % (target_width, target_height))
...
def SessionConvert(folder_name, opt, saving):
    ...
    if opt == 0 :
        ...
        for uid_file in all_uid_file:
            ...
            UidFileConvert(folder_name, uid_file, "_av", 0, offset_time)
